CLASS zcl_gtt_mia_ef_processor DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.

    INTERFACES zif_gtt_mia_ef_processor .

    METHODS constructor
      IMPORTING
        !io_ef_parameters TYPE REF TO zif_gtt_mia_ef_parameters
        !io_bo_reader     TYPE REF TO zif_gtt_mia_tp_reader
        !io_pe_filler     TYPE REF TO zif_gtt_mia_pe_filler
...
CLASS zcl_gtt_mia_ef_processor IMPLEMENTATION.


  METHOD add_struct_to_control_data.

    DATA: lt_fields       TYPE cl_abap_structdescr=>component_table,
          ls_control_data TYPE zif_gtt_mia_ef_types=>ts_control_data,
      ...
      IF <ls_mapping> IS ASSIGNED.
        ...
        LOOP AT lt_fields ASSIGNING FIELD-SYMBOL(<ls_fields>).
          ...
          IF <lv_value> IS ASSIGNED AND <lv_paramname> IS ASSIGNED.
            ...
            IF zcl_gtt_mia_tools=>is_table( iv_value = <lv_value> ) = abap_true.
          ...
          ELSEIF <lv_value> IS NOT ASSIGNED.
            MESSAGE e001(zgtt_mia) WITH <ls_fields>-name 'data table' INTO lv_dummy ##NO_TEXT.
            zcl_gtt_mia_tools=>throw_exception( ).
          ELSE.
            MESSAGE e001(zgtt_mia) WITH <ls_fields>-name 'mapping table' INTO lv_dummy ##NO_TEXT.
            zcl_gtt_mia_tools=>throw_exception( ).
          ENDIF.
        ENDLOOP.
      ELSE.
        MESSAGE e002(zgtt_mia) WITH 'mapping table' INTO lv_dummy ##NO_TEXT.
        zcl_gtt_mia_tools=>throw_exception( ).
      ENDIF.
    ELSE.
      MESSAGE e002(zgtt_mia) WITH 'data table' INTO lv_dummy ##NO_TEXT.
      zcl_gtt_mia_tools=>throw_exception( ).
    ENDIF.
  ...
  METHOD add_sys_attr_to_control_data.
    ...
    DATA: ls_control_data TYPE zif_gtt_mia_ef_types=>ts_control_data.
    ...
    ls_control_data-paramname   = zif_gtt_mia_ef_constants=>cs_system_fields-actual_bisiness_timezone.
    ls_control_data-value       = zcl_gtt_mia_tools=>get_system_time_zone( ).
    APPEND ls_control_data TO ct_control_data.

    ls_control_data-paramname   = zif_gtt_mia_ef_constants=>cs_system_fields-actual_bisiness_datetime.
    ls_control_data-value       = zcl_gtt_mia_tools=>get_system_date_time( ).
    APPEND ls_control_data TO ct_control_data.

    ls_control_data-paramname   = zif_gtt_mia_ef_constants=>cs_system_fields-actual_technical_timezone.
    ls_control_data-value       = zcl_gtt_mia_tools=>get_system_time_zone( ).
    APPEND ls_control_data TO ct_control_data.

    ls_control_data-paramname   = zif_gtt_mia_ef_constants=>cs_system_fields-actual_technical_datetime.
    ls_control_data-value       = zcl_gtt_mia_tools=>get_system_date_time( ).
    APPEND ls_control_data TO ct_control_data.
  ...
  METHOD zif_gtt_mia_ef_processor~check_app_objects.
      ...
      IF <ls_app_objects>-maintabdef <> ms_definition-maintab.
        MESSAGE e087(/saptrx/asc)
          WITH <ls_app_objects>-maintabdef
               ...
               zif_gtt_mia_ef_constants=>cv_aot
               mo_ef_parameters->get_appsys( )
          INTO lv_dummy.

        zcl_gtt_mia_tools=>throw_exception(
          iv_textid = zif_gtt_mia_ef_constants=>cs_errors-table_determination ).
  ...
  METHOD zif_gtt_mia_ef_processor~check_relevance.
  ...
  METHOD zif_gtt_mia_ef_processor~get_control_data.
    ...
    DATA: lt_control_data TYPE zif_gtt_mia_ef_types=>tt_control_data,
  ...
  METHOD zif_gtt_mia_ef_processor~get_planned_events.

    DATA: lt_expeventdata TYPE zif_gtt_mia_ef_types=>tt_expeventdata,
          lt_measrmntdata TYPE zif_gtt_mia_ef_types=>tt_measrmntdata,
          lt_infodata     TYPE zif_gtt_mia_ef_types=>tt_infodata,
          lr_app_objects  TYPE REF TO data.
  ...
  METHOD zif_gtt_mia_ef_processor~get_track_id_data.
