CLASS zcl_gtt_mia_tools DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS convert_to_external_amount
      ...
      RETURNING
        VALUE(rv_external) TYPE zif_gtt_mia_ef_types=>tv_currency_amnt .
    ...
    CLASS-METHODS is_object_changed
      IMPORTING
        ...
        !it_check_tables  TYPE zif_gtt_mia_ef_types=>tt_strucdatadef OPTIONAL
...
CLASS zcl_gtt_mia_tools IMPLEMENTATION.
  ...
  METHOD are_fields_different.
    ...
    LOOP AT it_fields ASSIGNING FIELD-SYMBOL(<lv_field>).
      ...
      ELSE.
        MESSAGE e001(zgtt_mia) WITH <lv_field> '' INTO DATA(lv_dummy).
    ...
    IF <ls_data1> IS ASSIGNED AND <ls_data2> IS ASSIGNED.
      ...
      LOOP AT lt_fields ASSIGNING FIELD-SYMBOL(<ls_fields>).
        ...
        IF <lv_value1> IS ASSIGNED AND
          ...
          ENDIF.
        ELSE.
          MESSAGE e001(zgtt_mia) WITH <ls_fields>-name '' INTO lv_dummy.
          zcl_gtt_mia_tools=>throw_exception( ).
        ENDIF.
      ENDLOOP.
    ELSE.
      MESSAGE e002(zgtt_mia) WITH '' INTO lv_dummy.
      zcl_gtt_mia_tools=>throw_exception( ).
    ENDIF.
  ...
  METHOD get_field_of_structure.
    ...
    IF <ls_struct> IS ASSIGNED.
      ...
      IF <lv_value> IS ASSIGNED.
        rv_value    = <lv_value>.
      ELSE.
        MESSAGE e001(zgtt_mia) WITH iv_field_name '' INTO lv_dummy.
        zcl_gtt_mia_tools=>throw_exception( ).
      ENDIF.
    ELSE.
      MESSAGE e002(zgtt_mia) WITH '' INTO lv_dummy.
  ...
  METHOD get_pretty_value.
    ...
    rv_pretty   = COND #( WHEN zcl_gtt_mia_tools=>is_date( iv_value = iv_value ) = abap_true
                            THEN COND #( WHEN iv_value IS NOT INITIAL
                                           THEN iv_value
                                           ELSE '' )
                          WHEN zcl_gtt_mia_tools=>is_timestamp( iv_value = iv_value ) = abap_true
                            THEN COND #( WHEN iv_value IS NOT INITIAL
                                           THEN get_valid_datetime( iv_timestamp = iv_value )
                                           ELSE '' )
                          WHEN zcl_gtt_mia_tools=>is_number( iv_value = iv_value ) = abap_true
                            THEN |{ iv_value }|
                            ELSE iv_value ).
  ...
  METHOD get_system_time_zone.
    ...
    CALL FUNCTION 'GET_SYSTEM_TIMEZONE'
      ...
      EXCEPTIONS
        ...
        OTHERS              = 2.
    IF sy-subrc <> 0.
      MESSAGE e003(zgtt_mia) INTO DATA(lv_dummy).
      zcl_gtt_mia_tools=>throw_exception( ).
    ENDIF.
  ...
  METHOD is_structure_changed.
    ...
    IF <ls_struct> IS ASSIGNED.
      ...
      ELSE.
        MESSAGE e001(zgtt_mia) WITH iv_upd_field '' INTO lv_dummy.
        zcl_gtt_mia_tools=>throw_exception( ).
      ENDIF.
    ELSE.
      MESSAGE e002(zgtt_mia) WITH '' INTO lv_dummy.
    ...
    IF <lt_table> IS ASSIGNED.
      LOOP AT <lt_table> ASSIGNING <ls_table>.
        ...
        ELSE.
          ...
          MESSAGE e001(zgtt_mia) WITH lv_fname '' INTO lv_dummy.
          zcl_gtt_mia_tools=>throw_exception( ).
        ENDIF.
      ENDLOOP.
    ELSE.
  ...
  METHOD is_timestamp.
    ...
    IF lo_type IS INSTANCE OF cl_abap_elemdescr.
      ...
      rv_result   = boolc( lo_elem->help_id = zif_gtt_mia_ef_constants=>cs_date_types-timestamp ).
    ELSE.
      rv_result   = abap_false.
    ENDIF.
