...
private section.
  ...
  methods ADD_REQUIRED_FREIGHT_UNIT
    ...
    raising
      /SCMTMS/CX_EVENT_MANAGEMENT .
  methods ADD_REQUIRED_FREIGHT_ORDER
    changing
      !CT_EXECINFO_SSTRING type /SCMTMS/T_EM_BO_TOR_EXECINFO
      !CT_EXECINFO_BEFORE_SSTRING type /SCMTMS/T_EM_BO_TOR_EXECINFO
      !CT_TENDERING_SSTRING type /SCMTMS/T_EM_BO_TOR_TEND
...
*     4)life cycle status = cancel
  ...
  METHOD get_freight_order_info.
    ...
    DATA:
      ...
      lt_req_root                 TYPE /scmtms/t_tor_root_k,
      lt_req_root_before          TYPE /scmtms/t_tor_root_k,
      ...
      lt_req_root_sstring         TYPE /scmtms/t_em_bo_tor_root,
      lt_req_root_before_sstring  TYPE /scmtms/t_em_bo_tor_root,
    ...
    lo_tor_srv_mgr->retrieve_by_association(
      EXPORTING
        ...
        iv_fill_data    = abap_true
      IMPORTING
        et_target_key   = DATA(lt_req_root_key)
        et_data         = lt_req_root ).
    lt_req_root_sstring = CORRESPONDING #( lt_req_root MAPPING node_id = key ).
    INSERT LINES OF lt_req_root_sstring INTO TABLE ct_req_root_sstring.
    ...
    lo_tor_srv_mgr->retrieve_by_association(
      EXPORTING
        ...
        iv_fill_data    = abap_true
      IMPORTING
        et_data         = lt_req_root_before ).
    lt_req_root_before_sstring = CORRESPONDING #( lt_req_root_before MAPPING node_id = key ).
    INSERT LINES OF lt_req_root_before_sstring INTO TABLE ct_req_root_bef_sstring.

    lo_tor_srv_mgr->retrieve_by_association(
      EXPORTING
        iv_node_key     = /scmtms/if_tor_c=>sc_node-root
        iv_before_image = abap_false
        it_key          = lt_req_root_key
        iv_association  = /scmtms/if_tor_c=>sc_association-root-stop
        iv_fill_data    = abap_true
      IMPORTING
        et_data         = lt_req_stop ).
