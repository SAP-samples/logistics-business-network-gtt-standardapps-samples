CLASS zcl_gtt_spof_ae_fill_po_itm_de DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_GTT_SPOF_AE_FILL_PO_ITM_DE IMPLEMENTATION.
  ...
  METHOD zif_gtt_ae_filler~check_relevance.
    ...
    IF is_events-maintabdef = zif_gtt_spof_app_constants=>cs_tabledef-po_item_new AND
      ...
      rv_result = COND #( WHEN lv_loekz_new <> lv_loekz_old AND
                             ( lv_loekz_old = zif_gtt_spof_app_constants=>cs_loekz-deleted OR
                               lv_loekz_new = zif_gtt_spof_app_constants=>cs_loekz-deleted )
  ...
  METHOD zif_gtt_ae_filler~get_event_data.
    DATA(lv_loekz) = CONV eloek( zcl_gtt_tools=>get_field_of_structure(
                                   ir_struct_data = is_events-maintabref
                                   iv_field_name  = 'LOEKZ' ) ).
    ...
    ct_trackingheader = VALUE #( BASE ct_trackingheader (
      ...
      evtid       = COND #( WHEN lv_loekz = zif_gtt_spof_app_constants=>cs_loekz-deleted
                              THEN zif_gtt_ef_constants=>cs_milestone-po_deletion
                              ELSE zif_gtt_ef_constants=>cs_milestone-po_undeletion )
      evtdat      = sy-datum
      evttim      = sy-uzeit
