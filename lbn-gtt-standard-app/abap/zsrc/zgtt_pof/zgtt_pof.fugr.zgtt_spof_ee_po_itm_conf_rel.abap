FUNCTION ZGTT_SPOF_EE_PO_ITM_CONF_REL.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  TRY.
      e_result  = zcl_gtt_ae_performer=>check_relevance(
        EXPORTING
          is_definition       = VALUE #(
                                  maintab   = zif_gtt_spof_app_constants=>cs_tabledef-po_item_new
                                  mastertab = zif_gtt_spof_app_constants=>cs_tabledef-po_header_new )
          io_ae_factory       = NEW zcl_gtt_spof_ae_fact_po_itm_cf( )
          iv_appsys           = i_appsys
