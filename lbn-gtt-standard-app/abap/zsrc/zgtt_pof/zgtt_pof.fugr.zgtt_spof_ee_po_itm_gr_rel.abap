FUNCTION ZGTT_SPOF_EE_PO_ITM_GR_REL.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  TRY.
      e_result  = zcl_gtt_ae_performer=>check_relevance(
        EXPORTING
          is_definition       = VALUE #(
                                  maintab   = zif_gtt_spof_app_constants=>cs_tabledef-md_material_segment
                                  mastertab = zif_gtt_spof_app_constants=>cs_tabledef-md_material_header )
          io_ae_factory       = NEW zcl_gtt_spof_ae_fact_po_itm_gr( )
          iv_appsys           = i_appsys
